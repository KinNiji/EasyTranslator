...
export default function Home() {
  ...
  return <main className="app-shell" lang={uiLanguage}>
    <header className="topbar"><div className="brand-lockup"><Image className="institute-logo" src={`${assetBasePath}/brand/institute-logo.png`} alt={t('instituteAlt')} width={388} height={95} priority /><div className="title-block"><p className="eyebrow">EASYTRANSLATOR · P2</p><h1>{t('appName')}</h1><p className="brand-tagline">{t('brandTagline')}</p></div></div><div className="top-actions"><button className="text-button help-button" onClick={() => setHelpOpen(true)}>{t('help')}</button><button className="icon-button" aria-label={theme === 'dark' ? t('light') : t('dark')} onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? '☀' : '◐'}</button><button className="icon-button" aria-label={t('apiSettings')} onClick={() => setSettingsOpen(true)}>⚙</button></div></header>
      ...
      <section className="conversation-panel">
        {!activeConversation || !activeUtterance ? <div className="empty-state"><p>{t('startConversation')}</p><button className="primary-button" onClick={createNewConversation} disabled={!acknowledged}>{t('newConversation')}</button></div> : <>
          ...
          <article className="utterance-card">
            ...
            <div className="language-picker" aria-label={t('source')}><label className="language-select"><span>{t('source')}</span><select value={activeUtterance.source.language} onChange={(event) => setSourceLanguage(event.target.value as Language)} disabled={!acknowledged || isBusy || recording}>{(['und', 'zh', 'fr', 'en'] as Language[]).map((language) => <option key={language} value={language}>{languageFlags[language]} {spokenLanguageLabels[uiLanguage][language]}</option>)}</select></label><button className="confirm-button" onClick={() => void translateCurrent()} disabled={!acknowledged || isBusy || recording}>{busy === 'translating' ? t('translating') : t('detectTranslate')}</button></div>
