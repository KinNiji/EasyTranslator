...
- **Word / PDF：** 使用同一份导出中间数据生成，包含标题、导出时间、句号、发言者与来源语言标签。Word 为可编辑 `.docx`；PDF 在浏览器本地按系统中法英字体排版并栅格化为页面图像，以优先保证中文和法语重音在移动端正确显示（PDF 正文不保证可选中）。
...
- 已完成：用量事件、发言/对话汇总、JSON 备份导入、指定语言 Markdown 导出，以及 Word/PDF 导出。导入会重新生成记录与发言 ID，只读取对话、翻译和用量字段，明确忽略任意设置或 API Key 字段。设置中提供会话内可编辑的价目表，价格不会持久化，刷新后恢复默认值。
- 待完成：费用计算单元测试、Playwright 关键移动流程与屏幕阅读器检查。
