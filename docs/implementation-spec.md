...
- 已完成：用量事件、发言/对话汇总、JSON 备份导入、指定语言 Markdown 导出，以及 Word/PDF 导出。导入会重新生成记录与发言 ID，只读取对话、翻译和用量字段，明确忽略任意设置或 API Key 字段。
- 待完成：可编辑价目表、费用计算单元测试、Playwright 关键移动流程与屏幕阅读器检查。
